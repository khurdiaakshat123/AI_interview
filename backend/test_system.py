...
def run_tests():
    ...
    res = httpx.get(f"{BASE_URL}/role-profiles")
    ...
    res1 = httpx.post(f"{BASE_URL}/practice/sessions", json=p_payload)
    ...
    res2 = httpx.post(f"{BASE_URL}/practice/sessions", json=p_payload)
    session2 = res2.json()
    ...
    ans_res = httpx.post(f"{BASE_URL}/practice/questions/{q_id}/check-answer", json={"question_id": q_id, "answer": "def lengthOfLongestSubstring(s):\n    return 3"})
    assert ans_res.status_code == 200
    print(f"[PASS] Evaluator Registry Check Answer: is_correct={ans_res.json()['is_correct']}, score={ans_res.json()['score_fraction']}")

    hint_res = httpx.get(f"{BASE_URL}/practice/questions/{q_id}/hint")
    assert hint_res.status_code == 200
    print(f"[PASS] Agent 2 Hint: {hint_res.json()['hint'][:50]}...")

    sol_res = httpx.get(f"{BASE_URL}/practice/questions/{q_id}/solution")
    ...
    oa_res = httpx.post(f"{BASE_URL}/mock-oa/start", json={"company": "Google", "role": "Software Engineer II (L4)"})
    ...
    sub_res = httpx.post(f"{BASE_URL}/mock-oa/attempts/{attempt_id}/submit", json={
    ...
    int_start = httpx.post(f"{BASE_URL}/interview/sessions", json={
        "company": "Google",
        "role": "Software Engineer II (L4)",
    ...
    ans_turn = httpx.post(f"{BASE_URL}/interview/sessions/{turn1['session_id']}/answer", json={
    ...
    final_rep_res = httpx.post(f"{BASE_URL}/interview/sessions/{turn1['session_id']}/answer", json={
    ...
    rep_res = httpx.get(f"{BASE_URL}/interview/sessions/{turn1['session_id']}/report")
    ...
    rev_res = httpx.get(f"{BASE_URL}/admin/review-queue")
